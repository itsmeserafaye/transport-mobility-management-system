...
<body class="bg-slate-50 dark:bg-slate-900">
    ...
    <script>
        ...
        async function loadMonthlyChart() {
            try {
                const response = await fetch('http://localhost:3000/api/v1/violations/revenue/summary?group_by=month');
                const result = await response.json();
                
                if (result.success && result.data && result.data.data) {
                    renderMonthlyChart(result.data.data);
                } else {
        ...
        function renderMonthlyChart(data) {
            ...
            const maxValue = Math.max(...data.map(item => parseFloat(item.collected_amount || 0)));
            const scaleFactor = maxValue > 0 ? maxValue : 1;
            
            // Generate chart bars
            let chartHTML = '';
            data.forEach(item => {
                const amount = parseFloat(item.collected_amount || 0);
                const height = maxValue > 0 ? Math.max(5, (amount / scaleFactor) * 100) : 5;
                const monthName = new Date(item.period + '-01').toLocaleDateString('en-US', { month: 'short' });
