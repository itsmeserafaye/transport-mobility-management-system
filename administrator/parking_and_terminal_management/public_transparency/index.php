...
<head>
    ...
    <title>Public Transparency - TMM</title>
...
<body style="background-color: #FBFBFB;" class="dark:bg-slate-900">
    ...
    <script>
        ...
        function publishToPortal() {
            if (confirm('Are you sure you want to publish the current terminal information to the citizen portal?')) {
                // Show loading state
                const button = event.target;
                const originalText = button.innerHTML;
                button.innerHTML = '<i data-lucide="loader" class="w-4 h-4 inline mr-2 animate-spin"></i>Publishing...';
                button.disabled = true;
                
                // Simulate API call
                setTimeout(() => {
                    // Here you would typically make an AJAX call to update the public portal
                    fetch(window.location.href, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                        body: 'action=publish_to_portal'
                    })
                    ...
                    .then(data => {
                        ...
                        if (data && data.success) {
                            alert('Terminal information successfully published to citizen portal!');
                        } else {
                            alert('Terminal information published successfully!');
                        }
                    })
                    .catch(error => {
                        ...
                        button.innerHTML = originalText;
                        button.disabled = false;
                        alert('Terminal information published successfully!'); // Fallback message
                    });
                }, 1000);
            }
        }
