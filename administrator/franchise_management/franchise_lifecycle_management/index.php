...
<body style="background-color: #FBFBFB;" class="dark:bg-slate-900">
            ...
            <nav class="p-4 space-y-2">
                ...
                <div class="space-y-1">
                    ...
                    <div id="franchise-mgmt-menu" class="ml-8 space-y-1">
                        ...
                        <a href="../franchise_lifecycle_management/" class="block p-2 text-sm rounded-lg font-medium" style="color: #4CAF50; background-color: rgba(76, 175, 80, 0.2);">Franchise Lifecycle Management</a>
                                ...
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex space-x-2">
                                            ...
                                            <?php if ($lc['lifecycle_stage'] == 'renewal' || $lc['action_required'] == 'renewal'): ?>
                                            <button onclick="processRenewal('<?php echo $lc['lifecycle_id']; ?>')" class="p-1 text-green-600 hover:bg-green-100 rounded" title="Process Renewal">
                                                <i data-lucide="refresh-cw" class="w-4 h-4"></i>
                                            </button>
                                            <?php endif; ?>
                                            <?php if ($lc['lifecycle_stage'] == 'active'): ?>
                                            <button onclick="amendFranchise('<?php echo $lc['lifecycle_id']; ?>')" class="p-1 text-orange-600 hover:bg-orange-100 rounded" title="Amend">
                                                <i data-lucide="edit" class="w-4 h-4"></i>
                                            </button>
                                            <button onclick="suspendFranchise('<?php echo $lc['lifecycle_id']; ?>')" class="p-1 text-yellow-600 hover:bg-yellow-100 rounded" title="Suspend">
                                                <i data-lucide="pause" class="w-4 h-4"></i>
                                            </button>
                                            <button onclick="revokeFranchise('<?php echo $lc['lifecycle_id']; ?>')" class="p-1 text-red-600 hover:bg-red-100 rounded" title="Revoke">
    ...
    <script>
        ...
        function processRenewal(lifecycleId) {
            const modal = document.createElement('div');
            modal.className = 'fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center';
            modal.innerHTML = `
                <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-xl font-bold">Process Renewal</h2>
                        <button onclick="this.closest('.fixed').remove()" class="text-gray-500 hover:text-gray-700">
                            <i data-lucide="x" class="w-6 h-6"></i>
                        </button>
                    </div>
                    <form id="renewalForm">
                        <div class="mb-4">
                            ...
                            <input type="date" id="renewalDate" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-300" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Processed By</label>
                            <input type="text" id="processedBy" value="Admin" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-300" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Remarks (Optional)</label>
                            <textarea id="renewalRemarks" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-300" rows="3" placeholder="Enter any additional remarks..."></textarea>
                        </div>
                        <div class="flex space-x-3">
        ...
        function amendFranchise(lifecycleId) {
            const modal = document.createElement('div');
            modal.className = 'fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center';
            modal.innerHTML = `
                <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
                    <div class="flex justify-between items-center mb-4">
                        <h2 class="text-xl font-bold">Amend Franchise</h2>
                        <button onclick="this.closest('.fixed').remove()" class="text-gray-500 hover:text-gray-700">
                            <i data-lucide="x" class="w-6 h-6"></i>
                        </button>
                    </div>
                    <form id="amendmentForm">
                        <div class="mb-4">
                            ...
                            <select id="amendmentType" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-300" required>
                                ...
                                <option value="schedule_modification">Schedule Modification</option>
                                <option value="other">Other</option>
                            </select>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Reason for Amendment</label>
                            <textarea id="amendmentReason" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-300" rows="3" placeholder="Enter reason for amendment..." required></textarea>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Processed By</label>
                            <input type="text" id="amendmentProcessedBy" value="Admin" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-300" required>
                        </div>
                        <div class="flex space-x-3">
                            <button type="button" onclick="this.closest('.fixed').remove()" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50">
                                Cancel
            ...
            document.getElementById('amendmentForm').addEventListener('submit', function(e) {
                ...
                const processedBy = document.getElementById('amendmentProcessedBy').value;
                
                fetch('handler.php', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: new URLSearchParams({
                        action: 'amend_franchise',
                        lifecycle_id: lifecycleId,
                ...
                .then(data => {
                    if (data.success) {
                        alert('Amendment processed successfully!');
                        modal.remove();
                        location.reload();
                    } else {
                ...
                .catch(error => {
                    alert('Error processing amendment');
                });
            });
        }
        
        function suspendFranchise(lifecycleId) {
            const reason = prompt('Enter reason for suspension:');
            if (reason && confirm('Suspend this franchise?')) {
                fetch('handler.php', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: new URLSearchParams({
                        action: 'suspend_franchise',
                        lifecycle_id: lifecycleId,
                        reason: reason,
                        processed_by: 'Admin'
                    })
                ...
                .then(data => {
                    if (data.success) {
                        alert('Franchise suspended successfully!');
                        location.reload();
                    } else {
                ...
                .catch(error => {
                    alert('Error suspending franchise');
                });
            }
        }
        
        function revokeFranchise(lifecycleId) {
            const reason = prompt('Enter reason for revocation:');
            if (reason && confirm('Revoke this franchise? This action cannot be undone.')) {
                fetch('handler.php', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/x-www-form-urlencoded'},
                    body: new URLSearchParams({
                        action: 'revoke_franchise',
                        lifecycle_id: lifecycleId,
                        reason: reason,
                        processed_by: 'Admin'
                    })
                ...
                .then(data => {
                    if (data.success) {
                        alert('Franchise revoked successfully!');
                        location.reload();
                    } else {
                ...
                .catch(error => {
                    alert('Error revoking franchise');
                });
            }
        }
